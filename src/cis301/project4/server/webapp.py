...
def del_phonecall(phonecall_id=None):
    if request.method == 'GET':
        return '{"res":"401", "text":"Unauthorized client error"}', 401
    if not (session.get('user') and session.get('uid')):
        return '{"res":"401", "text":"Unauthorized access"}', 401

    data = request.json
    phonecall_id = data['phonecall_id']
    if phonecall_id:
        if app.get_userdb().delete_phonecall(phonecall_id,session.get('uid')):
            return '{"res":"200", "text":"Deleted"}', 200
        else:
            return '{"res":"401", "text":"Could not delete {phonecall_id}"}', 200
